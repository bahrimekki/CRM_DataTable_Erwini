...
// --- Sidebare liste Display Function ---
const loadSectionData = async (table) => {
    const section = await getItems(table);
    //moment(data.datetime).format("YYYY-DD-MM HH:mm") === dayjs().format(TO_PATTERN))
    //console.log("section : ", section)

    // Étape 1 : Grouper les zones par propriétaire
    const groupedData = section.reduce((acc, item) => {
        // Si le propriétaire n'existe pas encore dans l'accumulateur, on l'ajoute
        if (!acc[item.owner]) {
    ...
    // Étape 2 : Convertir l'objet groupé en tableau
    const result = Object.values(groupedData).reverse();

    //console.log(result);
    
    const sidebar = document.getElementById('mainList');

    result.forEach((item) => {
    const li = document.createElement('li');
    li.innerHTML = `
        <div class="collapsible">
            <i class="fas fa-wifi icon" style="color:${item.etat==="on"?"#00ff00":"#ff0000"}"></i>
            <span>${item.owner}</span>
            <span class="child-count">(${item.zones.length})</span>
    ...
    // Ajouter les écouteurs d'événements pour les éléments collapsibles
